...
void QwiicOTOS::resetTracking() {
    writeByte(kRegReset, 0x01);
}

// Lecture de la configuration du traitement du signal
int QwiicOTOS::getSignalProcessConfig() {
    return readByte(kRegSignalProcess); // Utilisation de la méthode i2cReadByte pour lire un octet du registre
}

// Écriture de la configuration du traitement du signal
void QwiicOTOS::setSignalProcessConfig(int config) {
    writeByte(kRegSignalProcess, static_cast<uint8_t>(config)); // Conversion du config en uint8_t et écriture dans le registre
}

// Lecture des offsets de position (en mètres et radians)
Pose2D QwiicOTOS::getOffset() {
    return readPoseRegs(kRegOffXL, kInt16ToMeter, kInt16ToRad); // Appel à la méthode de lecture des registres
}

// Écriture des offsets de position
bool QwiicOTOS::setOffset(const Pose2D& pose) {
    return writePoseRegs(kRegOffXL, pose, kMeterToInt16, kRadToInt16); // Conversion et écriture des données d'offset
}

// Lecture de la position (en mètres et radians)
Pose2D QwiicOTOS::getPosition() {
    return readPoseRegs(kRegPosXL, kInt16ToMeter, kInt16ToRad); // Lecture de la position depuis les registres
}

// Écriture de la position (en mètres et radians)
void QwiicOTOS::setPosition(const Pose2D& pose) {
    writePoseRegs(kRegPosXL, pose, kMeterToInt16, kRadToInt16); // Conversion et écriture de la nouvelle position
}
...
bool QwiicOTOS::writeByte(uint8_t reg, uint8_t data) {
    return HAL_I2C_Mem_Write(hi2c_, address << 1, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, HAL_MAX_DELAY) == HAL_OK;
}

uint8_t QwiicOTOS::readByte(uint8_t reg) {
    uint8_t data = 0;
    HAL_I2C_Mem_Read(hi2c_, address << 1, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, HAL_MAX_DELAY);
    return data;
}

bool QwiicOTOS::readBlock(uint8_t reg, uint8_t *data, uint16_t length) {
    return HAL_I2C_Mem_Read(hi2c_, address << 1, reg, I2C_MEMADD_SIZE_8BIT, data, length, HAL_MAX_DELAY) == HAL_OK;
}

bool QwiicOTOS::writeBlock(uint8_t reg, uint8_t* data, uint16_t length) {
    return HAL_I2C_Mem_Write(hi2c_, address << 1, reg, I2C_MEMADD_SIZE_8BIT, data, length, HAL_MAX_DELAY) == HAL_OK;
}
...
Pose2D QwiicOTOS::regsToPose(const uint8_t* rawData, float rawToXY, float rawToH) {
    int16_t rawX = (rawData[1] << 8) | rawData[0];
    int16_t rawY = (rawData[3] << 8) | rawData[2];
    int16_t rawH = (rawData[5] << 8) | rawData[4];
    return Pose2D(rawX * rawToXY * meterToUnit, rawY * rawToXY * meterToUnit, rawH * rawToH * radToUnit);
}

void QwiicOTOS::poseToRegs(const Pose2D& pose, uint8_t* rawData, float xyToRaw, float hToRaw)
{
...
int QwiicOTOS::writePoseRegs(uint8_t reg, const Pose2D &pose, float xyToRaw, float hToRaw)
{
